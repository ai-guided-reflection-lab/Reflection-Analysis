...
from application.html_builder.chi_square_helper import ChiSquareTableHelper
from application.html_builder.grade_module_correlation_helper import GradeModuleCorrelationHelper
from application.html_builder.builders.emotions_analysis_builder import EmotionsAnalysisBuilder
from application.html_builder.builders.ssm_before_after_builder import SSMBeforeAfterBuilder
import collections
import csv
...
class HTMLBuilder:
    def __init__(self):
        ...
        self.ssm_before_after_builder = SSMBeforeAfterBuilder()
    ...
    def build_student_profiles(self, course: Course, mode: str = "full") -> str:
        """
        Build HTML for all student profiles in a course
        
        Args:
            course: Course object containing student data
            mode: Report mode - "full" (with statistics) or "instructor" (simplified)
        """
        ...
        # Debug student data
        # Students list will be filtered later to only include reflection submitters with email IDs
        students = []  # Initialized here, filtered later after reflection analysis
        print(f"Total students in course roster: {len(course.students)}")
        ...
        try:
            ...
            # Pass chi-square explanation as a separate variable for the template
            chi_square_explanation_html = self._chi_square_explanation_block()
            ...
            for ref, path in reflection_files:
                if os.path.exists(path):
                    ...
                    summary_htmls[ref] = f'''<div id="summary-div-{ref}" style="background:#f8f9fa;padding:10px 18px;margin-bottom:12px;border-radius:6px;font-size:1.1em;{'display:none;' if ref != selected_ref else ''}"><b>Summary for Reflection {ref}:</b> <span style='margin-left:18px;'>Total students in course: <b>{total_students_in_course}</b></span> <span style='margin-left:18px;'>Total students submitted: <b>{total_students_submitted}</b></span></div>'''
                else:
                    summary_htmls[ref] = f'<div id="summary-div-{ref}" style="background:#f8f9fa;padding:10px 18px;margin-bottom:12px;border-radius:6px;font-size:1.1em;display:none;"><b>Summary for Reflection {ref}:</b> <span style="margin-left:18px;">Data not available</span></div>'
                ...
                if os.path.exists(csv_path):
                    ...
                    if 'ID' in df.columns:
                        for _, row in df.iterrows():
                            ...
                            # Only count email-based IDs
                            if '@' in student_id:
                                email_submitters.add(student_id)
            
            # Set the correct email student count for students who submitted reflections
            email_student_count = len(email_submitters)
            course.set_num_students_with_email_id(email_student_count)
            
            # Create Student objects for email submitters that don't exist in course.students
            from application.model.models.student import Student
            for email_id in email_submitters:
                if email_id not in course.students:
                    # Create a Student object with the email ID
                    # Extract name from email prefix if needed
                    username = email_id.split('@')[0]
                    student = Student(
                        name=f"Student {username}",  # Placeholder name
                        email=email_id,
                        course=course.course_name
                    )
                    course.add_student(student)
            
            # Now populate ALL students from course roster for the dropdown (not just reflection submitters)
            # First, get all students from the latest grades file to ensure we have the complete roster
            latest_ref = max(ref_nums) if ref_nums else 1
            latest_grades_path = None
            for ref_num, csv_path in reflection_files:
                if ref_num == latest_ref:
                    dir_path = os.path.dirname(csv_path)
                    # Look for grades file
                    for fname in os.listdir(dir_path):
                        if fname.endswith(f'_grades_ref{ref_num}.csv') or fname.endswith(f'grades_ref{ref_num}.csv'):
                            latest_grades_path = os.path.join(dir_path, fname)
                            break
                    if not latest_grades_path:
                        results_dir = os.path.join(dir_path, 'results')
                        if os.path.exists(results_dir):
                            for fname in os.listdir(results_dir):
                                if fname.endswith(f'_grades_ref{ref_num}.csv') or fname.endswith(f'grades_ref{ref_num}.csv'):
                                    latest_grades_path = os.path.join(results_dir, fname)
                                    break
                    break
            
            # Load all students from the course roster (grades file) - ONLY ONCE
            if latest_grades_path and os.path.exists(latest_grades_path):
                try:
                    grades_df = pd.read_csv(latest_grades_path)
                    print(f"DEBUG: Loading course roster from {latest_grades_path}")
                    print(f"DEBUG: Raw grades file has {len(grades_df)} rows")
                    
                    if 'ID' in grades_df.columns:
                        # Remove rows with empty IDs and test students
                        grades_df = grades_df[grades_df['ID'].notna() & (grades_df['ID'].astype(str).str.strip() != '')]
                        mask = ~grades_df['ID'].astype(str).str.strip().str.lower().eq('student, test')
                        if 'Student' in grades_df.columns:
                            mask &= ~grades_df['Student'].astype(str).str.strip().str.lower().eq('student, test')
                        ...
                        print(f"DEBUG: After filtering, grades file has {len(grades_df)} unique students")
                        
                        # Create Student objects for all students in the roster
                        students_added = 0
                        for _, row in grades_df.iterrows():
                            student_id = str(row.get('ID', '')).strip()
                            if student_id and student_id not in course.students:
                                # Determine if this is an email or needs conversion
                                if '@' in student_id:
                                    email = student_id.lower()
                                    name = row.get('Student', f"Student {student_id.split('@')[0]}")
                                else:
                                    # For non-email IDs, create a placeholder student
                                    email = f"{student_id}@charlotte.edu"  # Convert to email format
                                    name = row.get('Student', f"Student {student_id}")
                                
                                try:
                                    student = Student(name=name, email=email, course=course.course_name)
                                    course.add_student(student)
                                    students_added += 1
                                except ValueError:
                                    # If email validation fails, skip this student
                                    print(f"Warning: Could not create Student object for ID {student_id}")
                                    continue
                        
                        print(f"DEBUG: Added {students_added} new students to course roster")
                        print(f"DEBUG: Total students in course.students: {len(course.students)}")
                        
                except Exception as e:
                    print(f"Warning: Could not load complete roster from {latest_grades_path}: {e}")
            
            # Include ALL students from course.students in the dropdown
            students = list(course.students.values())
            print(f"Total students in dropdown: {len(students)} (should equal course roster size)")
            print(f"Students who submitted reflections with email IDs: {email_student_count}")
            ...
            if csv_path:
                ...
                aggregates_html += '<div style="color:orange;">No grade data available - CSV generation may have failed.</div>'
            
            # --- Module Availability & Pre/Post Topic Analysis (move to bottom, ensure charts are included) ---
            if mode == "full":
                aggregates_html += self._build_module_availability_and_prepost_analysis(course, reflection_files, summary_topics, all_quizzes, student_topics, student_quizzes, student_topics_by_reflection, student_quizzes_by_reflection)
            ...
            for ref, path in reflection_files:
                ...
                if os.path.exists(path):
                    ...
                    for col in df.columns:
                        if col.strip().lower().startswith("how do you feel about the course so far?"):
                            emotion_col = col
                            break
                    if emotion_col:
                        ...
                        base_palette = {
                            'Happy': '#FFD700',        # Yellow (joy)
                            'Sad': '#90A4AE',          # Blue-gray (sadness)
            ...
            if len(ref_nums) > 1:
                ...
                if emotion_percent_data:
                    ...
                    fig_line.update_traces(
                        ...
                        customdata=percent_df[['Count']].values
                    )
            ...
            for topic in sorted(all_topics_set):
                for ref, path in reflection_files:
                    # Find all students who mentioned this topic in this reflection, and have an email ID
                    students_with_topic = [
                        sid for sid, ref_topics in student_topics_by_reflection.items()
                        if topic in ref_topics.get(ref, set()) and '@' in sid
                    ]
            ...
            # NEW: Generate chi-square HTML and store in variable for template
            topic_quiz_chi_square_html = self._build_topic_quiz_chi_square_table(student_topics, student_quizzes, summary_topics, all_quizzes)
            
            # Only include reverse chi-square analysis in full mode, not instructor mode
            if mode == "full":
                aggregates_html += self._build_quiz_topic_reverse_chi_square_table(student_topics, student_quizzes, summary_topics, all_quizzes)
            
            # Determine the base directory for calculation exports inside the model package
            calc_export_base = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'model', 'calculation_exports'))
            
            # --- Correlation between Reflection 4 grade and number of support modules completed ---
            if mode == "full":
                df_corr = GradeModuleCorrelationHelper.extract_grade_and_module_count(course, reflection_num=4)
                
                # Generate analysis with and without outliers for correlation
                result_html, plot_html, table_html, outlier_info_html = GradeModuleCorrelationHelper.compute_and_plot(
                    df_corr, 
                    course_name=getattr(course, 'course_name', 'unknown_course'),
                    export_base_dir=calc_export_base,
                    outlier_method=None
                )
                
                result_html_no_outliers, plot_html_no_outliers, table_html_no_outliers, outlier_info_html_iqr = GradeModuleCorrelationHelper.compute_and_plot(
                    df_corr, 
                    course_name=getattr(course, 'course_name', 'unknown_course'),
                    export_base_dir=calc_export_base,
                    outlier_method='iqr'
                ...
                result_html_threshold, plot_html_threshold, table_html_threshold, outlier_info_html_threshold = GradeModuleCorrelationHelper.compute_and_plot(
                    df_corr, 
                    course_name=getattr(course, 'course_name', 'unknown_course'),
                    export_base_dir=calc_export_base,
                ...
                aggregates_html += '''<details class="correlation-table-container" style="margin-top:32px;">
                    <summary class="section-header" style="font-size:1.2em;cursor:pointer;">
                        <span class="toggle-icon">▶</span>
                        <b>Correlation: Reflection 4 Grade vs. Number of Support Modules Completed</b>
                    </summary>
                    <div class="section-content" id="correlation-table-content">
                '''
                aggregates_html += GradeModuleCorrelationHelper.correlation_explanation()
                
                # Add three-way outlier toggle functionality
                aggregates_html += f'''
                    <div class="outlier-toggle-container" style="margin-bottom: 20px; padding: 15px; background-color: #f8f9fa; border-radius: 8px; border: 1px solid #dee2e6;">
                        <h4 style="margin-top: 0; color: #495057;">🎚️ Analysis Options</h4>
                        <div class="toggle-buttons" style="display: flex; gap: 10px; margin-bottom: 15px; flex-wrap: wrap;">
                            <button id="show-with-outliers-btn" onclick="showCorrelationAnalysis('with-outliers')" 
                                    class="toggle-btn active" style="padding: 8px 16px; border: 2px solid #007bff; background: #007bff; color: white; border-radius: 5px; cursor: pointer; font-weight: bold;">
                                📊 Include All Data
                            </button>
                            <button id="show-no-outliers-btn" onclick="showCorrelationAnalysis('no-outliers')" 
                                    class="toggle-btn" style="padding: 8px 16px; border: 2px solid #17a2b8; background: white; color: #17a2b8; border-radius: 5px; cursor: pointer; font-weight: bold;">
                                📈 Remove Statistical Outliers
                            </button>
                            <button id="show-threshold-btn" onclick="showCorrelationAnalysis('threshold')" 
                                    class="toggle-btn" style="padding: 8px 16px; border: 2px solid #ffc107; background: white; color: #ffc107; border-radius: 5px; cursor: pointer; font-weight: bold;">
                                🎯 Remove Grades &lt; 20
                            </button>
                        </div>
                        <div id="outlier-info" style="display: none;">
                            <div id="outlier-info-iqr" style="display: none;">{outlier_info_html_iqr}</div>
                            <div id="outlier-info-threshold" style="display: none;">{outlier_info_html_threshold}</div>
                        </div>
                    </div>
                    
                    <div id="correlation-with-outliers" class="correlation-analysis">
                        {result_html}
                        {plot_html}
                        <div style="margin-top:18px;">{table_html}</div>
                    </div>
                    
                    <div id="correlation-no-outliers" class="correlation-analysis" style="display: none;">
                        {result_html_no_outliers}
                        {plot_html_no_outliers}
                        <div style="margin-top:18px;">{table_html_no_outliers}</div>
                    </div>
                    
                    <div id="correlation-threshold" class="correlation-analysis" style="display: none;">
                        {result_html_threshold}
                        {plot_html_threshold}
                        <div style="margin-top:18px;">{table_html_threshold}</div>
                    </div>
                    
                    <script>
                        function showCorrelationAnalysis(mode) {{
                            // Update button states
                            const withOutliersBtn = document.getElementById('show-with-outliers-btn');
                            const noOutliersBtn = document.getElementById('show-no-outliers-btn');
                            const thresholdBtn = document.getElementById('show-threshold-btn');
                            const outlierInfo = document.getElementById('outlier-info');
                            const outlierInfoIqr = document.getElementById('outlier-info-iqr');
                            const outlierInfoThreshold = document.getElementById('outlier-info-threshold');
                            
                            // Reset button styles
                            withOutliersBtn.style.background = 'white';
                            withOutliersBtn.style.color = '#007bff';
                            noOutliersBtn.style.background = 'white';
                            noOutliersBtn.style.color = '#17a2b8';
                            thresholdBtn.style.background = 'white';
                            thresholdBtn.style.color = '#ffc107';
                            
                            // Hide all analysis divs
                            document.getElementById('correlation-with-outliers').style.display = 'none';
                            document.getElementById('correlation-no-outliers').style.display = 'none';
                            document.getElementById('correlation-threshold').style.display = 'none';
                            outlierInfoIqr.style.display = 'none';
                            outlierInfoThreshold.style.display = 'none';
                            
                            if (mode === 'with-outliers') {{
                                withOutliersBtn.style.background = '#007bff';
                                withOutliersBtn.style.color = 'white';
                                document.getElementById('correlation-with-outliers').style.display = 'block';
                                outlierInfo.style.display = 'none';
                            }} else if (mode === 'no-outliers') {{
                                noOutliersBtn.style.background = '#17a2b8';
                                noOutliersBtn.style.color = 'white';
                                document.getElementById('correlation-no-outliers').style.display = 'block';
                                outlierInfo.style.display = 'block';
                                outlierInfoIqr.style.display = 'block';
                            }} else if (mode === 'threshold') {{
                                thresholdBtn.style.background = '#ffc107';
                                thresholdBtn.style.color = 'white';
                                document.getElementById('correlation-threshold').style.display = 'block';
                                outlierInfo.style.display = 'block';
                                outlierInfoThreshold.style.display = 'block';
                            }}
                        }}
                        
                        // Initialize to show with outliers by default
                        document.addEventListener('DOMContentLoaded', function() {{
                            showCorrelationAnalysis('with-outliers');
                        }});
                    </script>
                    
                    </div></details>'''
                
                # --- NEW: Grade Distribution Analysis ---
                grade_distribution_html, grade_statistics_html = GradeModuleCorrelationHelper.create_grade_distribution_chart(
                    course, 
                    reflection_num=4,
                    course_name=getattr(course, 'course_name', 'unknown_course'),
                    export_base_dir=calc_export_base
                )
                aggregates_html += '''<details style="margin-top:32px;">
                    <summary style="font-size:1.2em;cursor:pointer;">
                        <b>Grade Distribution Analysis</b>
                    </summary>
                    <div style="margin:15px 0;">
                '''
                aggregates_html += grade_distribution_html
                aggregates_html += grade_statistics_html
                aggregates_html += '</div></details>'
                
                # --- NEW: Module Completion Distribution Analysis ---
                module_overall_html, module_individual_html, module_statistics_html = GradeModuleCorrelationHelper.create_module_completion_distribution_charts(
                    course,
                    course_name=getattr(course, 'course_name', 'unknown_course'),
                    export_base_dir=calc_export_base
                )
                aggregates_html += '''<details style="margin-top:32px;">
                    <summary style="font-size:1.2em;cursor:pointer;">
                        <b>Module Completion Distribution Analysis</b>
                    </summary>
                    <div style="margin:15px 0;">
                '''
                aggregates_html += module_overall_html
                aggregates_html += module_statistics_html
                aggregates_html += '<h4 style="margin-top:30px;color:#333;">Individual Module Completion Rates</h4>'
                aggregates_html += module_individual_html
                aggregates_html += '</div></details>'
                
                # --- Distribution Analysis Explanation ---
                aggregates_html += '''<details style="margin-top:32px;">
                    <summary style="font-size:1.2em;cursor:pointer;">
                        <b>Understanding Distribution Analysis and Normal Distribution</b>
                    </summary>
                    <div style="margin:15px 0;">
                '''
                aggregates_html += GradeModuleCorrelationHelper.distribution_explanation()
                aggregates_html += '</div></details>'
                
                # --- Correlation between number of modules completed and emotions ---
                corr_table_html, plots_html = GradeModuleCorrelationHelper.module_completion_emotion_correlation(
                    course,
                    reflection_files,
                    course_name=getattr(course, 'course_name', 'unknown_course'),
                    export_base_dir=calc_export_base
                )
                aggregates_html += '<details style="margin-top:32px;"><summary style="font-size:1.2em;cursor:pointer;"><b>Correlation: Number of Modules Completed vs. Emotions</b></summary>'
                aggregates_html += GradeModuleCorrelationHelper.module_emotion_correlation_explanation()
                aggregates_html += corr_table_html
                aggregates_html += plots_html
                aggregates_html += '</details>'
            
            # --- Grade comparison for each support module (Mann-Whitney U) ---
            if mode == "full":
                # Generate analysis with and without outliers
                completion_table_html, summary_html, boxplots_html, _ = GradeModuleCorrelationHelper.module_completion_grade_comparison(
                    course, 
                    reflection_num=4,
                    course_name=getattr(course, 'course_name', 'unknown_course'),
                    export_base_dir=calc_export_base,
                ...
                completion_table_html_no_outliers, summary_html_no_outliers, boxplots_html_no_outliers, mann_whitney_outlier_info_html_iqr = GradeModuleCorrelationHelper.module_completion_grade_comparison(
                    course, 
                    reflection_num=4,
                    course_name=getattr(course, 'course_name', 'unknown_course'),
                    export_base_dir=calc_export_base,
                ...
                completion_table_html_threshold, summary_html_threshold, boxplots_html_threshold, mann_whitney_outlier_info_html_threshold = GradeModuleCorrelationHelper.module_completion_grade_comparison(
                    course, 
                    reflection_num=4,
                    course_name=getattr(course, 'course_name', 'unknown_course'),
                    export_base_dir=calc_export_base,
                ...
                aggregates_html += '''<details style="margin-top:32px;">
                    <summary style="font-size:1.2em;cursor:pointer;">
                        <b>Grade Comparison: Completed vs. Not Completed (per Module)</b>
                    </summary>
                '''
                aggregates_html += GradeModuleCorrelationHelper.mannwhitney_explanation()
                
                # Add three-way outlier toggle functionality for Mann-Whitney analysis
                aggregates_html += f'''
                    <div class="outlier-toggle-container" style="margin-bottom: 20px; padding: 15px; background-color: #f8f9fa; border-radius: 8px; border: 1px solid #dee2e6;">
                        <h4 style="margin-top: 0; color: #495057;">🎚️ Analysis Options</h4>
                        <div class="toggle-buttons" style="display: flex; gap: 10px; margin-bottom: 15px; flex-wrap: wrap;">
                            <button id="show-mann-whitney-with-outliers-btn" onclick="showMannWhitneyAnalysis('with-outliers')" 
                                    class="toggle-btn active" style="padding: 8px 16px; border: 2px solid #28a745; background: #28a745; color: white; border-radius: 5px; cursor: pointer; font-weight: bold;">
                                📊 Include All Data
                            </button>
                            <button id="show-mann-whitney-no-outliers-btn" onclick="showMannWhitneyAnalysis('no-outliers')" 
                                    class="toggle-btn" style="padding: 8px 16px; border: 2px solid #17a2b8; background: white; color: #17a2b8; border-radius: 5px; cursor: pointer; font-weight: bold;">
                                📈 Remove Statistical Outliers
                            </button>
                            <button id="show-mann-whitney-threshold-btn" onclick="showMannWhitneyAnalysis('threshold')" 
                                    class="toggle-btn" style="padding: 8px 16px; border: 2px solid #ffc107; background: white; color: #ffc107; border-radius: 5px; cursor: pointer; font-weight: bold;">
                                🎯 Remove Grades &lt; 20
                            </button>
                        </div>
                        <div id="mann-whitney-outlier-info" style="display: none;">
                            <div id="mann-whitney-outlier-info-iqr" style="display: none;">{mann_whitney_outlier_info_html_iqr}</div>
                            <div id="mann-whitney-outlier-info-threshold" style="display: none;">{mann_whitney_outlier_info_html_threshold}</div>
                        </div>
                    </div>
                    
                    <div id="mann-whitney-with-outliers" class="mann-whitney-analysis">
                        {completion_table_html}
                        {summary_html}
                        {boxplots_html}
                    </div>
                    
                    <div id="mann-whitney-no-outliers" class="mann-whitney-analysis" style="display: none;">
                        {completion_table_html_no_outliers}
                        {summary_html_no_outliers}
                        {boxplots_html_no_outliers}
                    </div>
                    
                    <div id="mann-whitney-threshold" class="mann-whitney-analysis" style="display: none;">
                        {completion_table_html_threshold}
                        {summary_html_threshold}
                        {boxplots_html_threshold}
                    </div>
                    
                    <script>
                        function showMannWhitneyAnalysis(mode) {{
                            // Update button states
                            const withOutliersBtn = document.getElementById('show-mann-whitney-with-outliers-btn');
                            const noOutliersBtn = document.getElementById('show-mann-whitney-no-outliers-btn');
                            const thresholdBtn = document.getElementById('show-mann-whitney-threshold-btn');
                            const outlierInfo = document.getElementById('mann-whitney-outlier-info');
                            const outlierInfoIqr = document.getElementById('mann-whitney-outlier-info-iqr');
                            const outlierInfoThreshold = document.getElementById('mann-whitney-outlier-info-threshold');
                            
                            // Reset button styles
                            withOutliersBtn.style.background = 'white';
                            withOutliersBtn.style.color = '#28a745';
                            noOutliersBtn.style.background = 'white';
                            noOutliersBtn.style.color = '#17a2b8';
                            thresholdBtn.style.background = 'white';
                            thresholdBtn.style.color = '#ffc107';
                            ...
                            // Hide all analysis divs
                            document.getElementById('mann-whitney-with-outliers').style.display = 'none';
                            document.getElementById('mann-whitney-no-outliers').style.display = 'none';
                            document.getElementById('mann-whitney-threshold').style.display = 'none';
                            outlierInfoIqr.style.display = 'none';
                            outlierInfoThreshold.style.display = 'none';
                            
                            if (mode === 'with-outliers') {{
                                withOutliersBtn.style.background = '#28a745';
                                withOutliersBtn.style.color = 'white';
                                document.getElementById('mann-whitney-with-outliers').style.display = 'block';
                                outlierInfo.style.display = 'none';
                            }} else if (mode === 'no-outliers') {{
                                noOutliersBtn.style.background = '#17a2b8';
                                noOutliersBtn.style.color = 'white';
                                document.getElementById('mann-whitney-no-outliers').style.display = 'block';
                                outlierInfo.style.display = 'block';
                                outlierInfoIqr.style.display = 'block';
                            }} else if (mode === 'threshold') {{
                                thresholdBtn.style.background = '#ffc107';
                                thresholdBtn.style.color = 'white';
                                document.getElementById('mann-whitney-threshold').style.display = 'block';
                                outlierInfo.style.display = 'block';
                                outlierInfoThreshold.style.display = 'block';
                            }}
                        }}
                        ...
                        // Initialize to show with outliers by default
                        document.addEventListener('DOMContentLoaded', function() {{
                            showMannWhitneyAnalysis('with-outliers');
                        }});
                    </script>
                    
                    </details>'''
            
            # --- SSM Before/After Analysis ---
            if mode == "full":
                try:
                    ssm_analysis_html = self.ssm_before_after_builder.build_ssm_before_after_analysis(
                        course, 
                        reflection_files,
                        course_name=getattr(course, 'course_name', 'unknown_course'),
                        export_base_dir=calc_export_base
                    )
                    aggregates_html += ssm_analysis_html
                except Exception as e:
                    aggregates_html += f'<div style="color: red; margin-top: 20px;">Error generating SSM before/after analysis: {str(e)}</div>'
        ...
        try:
            html_content = template.render(
                ...
                topic_quiz_chi_square_html=topic_quiz_chi_square_html if mode == "full" else "",
                emotions_analysis_html=emotions_analysis_html,
                chi_square_explanation_html=chi_square_explanation_html if mode == "full" else "",
                statistical_comparison_html=statistical_comparison_html if mode == "full" else "",
                support_quiz_completion_table_html=support_quiz_completion_table_html if mode == "full" else "",
                quiz_completion_rate_chart_html=quiz_completion_rate_chart_html if mode == "full" else "",
                email_student_count=email_student_count  # Add this for JavaScript
            )
    ...
    def _build_emotions_topics_table(self, reflection_files, course):
        ...
        for ref_num, csv_path in reflection_files:
            if os.path.exists(csv_path):
                ...
                for col in df.columns:
                    if col.strip().lower().startswith("how do you feel about the course so far?"):
                        emotion_col = col
                    elif col.strip().lower().startswith("what topics would you like to discuss?"):
                        topic_col = col
                ...
                if emotion_col or topic_col:
                    for _, row in df.iterrows():
                        ...
                        # Enhanced validation: Only count email IDs for proper student matching
                        # 4-digit codes are kept for legacy compatibility but don't count as "matching"
                        is_email_id = '@' in student_id
                        ...
                        if is_valid_id:
                            ...
                            student_data[student_id][f'ref{ref_num}'] = {
                                ...
                                'is_email_id': is_email_id  # Track if this is an email-based ID
                            }
        ...
        # Calculate and set the correct count of students with email IDs
        students_with_email_ids = 0
        for student_id in student_data.keys():
            if '@' in student_id:  # Only count email-based IDs
                students_with_email_ids += 1
        
        # Update the course with the correct email ID count
        course.set_num_students_with_email_id(students_with_email_ids)
        print(f"Set course email ID student count to: {students_with_email_ids}")
        
        # Add summary info to the HTML showing the corrected counts
        html += f'''
        </div>
        <div style="margin-top:20px; padding:10px; background-color:#f8f9fa; border-radius:5px;">
            <h4>Student Matching Summary</h4>
            <p><strong>Total students in course:</strong> {course.filtered_total_students}</p>
            <p><strong>Students with valid email IDs:</strong> {students_with_email_ids}</p>
            <p><strong>Students using 4-digit codes:</strong> {len(student_data) - students_with_email_ids}</p>
            <p><em>Note: Only email-based IDs can be reliably matched to specific students in the grade data.</em></p>
        </div>
        '''
    ...
    def _build_topic_quiz_chi_square_table(self, student_topics, student_quizzes, summary_topics, all_quizzes):
        results = ChiSquareTableHelper.build_topic_quiz_chi_square(student_topics, student_quizzes, summary_topics, all_quizzes)
        html = '<div class="table-responsive">'
        html += '<table class="emotions-topics-table"><thead><tr><th>Topic</th><th>Quiz</th><th>p-value</th><th>Effect Size (Odds Ratio)</th><th>Direction</th><th>Contingency Table</th></tr></thead><tbody>'
        for topic, quiz, p, table, or_value in results:
            if or_value > 1:
                direction = '<span style="color:#1976d2;">&#8593; More likely</span>'
            elif or_value < 1:
                direction = '<span style="color:#d32f2f;">&#8595; Less likely</span>'
            else:
                direction = 'No effect'
            or_html = f'<span title="Odds Ratio quantifies how much more likely (or less likely) students mentioning the topic are to complete the quiz compared to those who do not. OR > 1: more likely, OR < 1: less likely.">{or_value:.2f}</span>'
            subtable = ChiSquareTableHelper.render_contingency_subtable(
                table,
                row_labels=["Mentioned Topic", "Did Not Mention Topic"],
                col_labels=["Completed Quiz", "Did Not Complete Quiz"]
            )
            html += f'<tr><td>{topic}</td><td>{quiz}</td><td>{p:.5f}</td><td>{or_html}</td><td>{direction}</td><td>{subtable}</td></tr>'
        html += '</tbody></table>'
        html += '</div>'
        html += self._chi_square_legend_block()
        return html

    def _chi_square_explanation_block(self):
        return '''<div style="margin-bottom:16px; font-size:1.1em;">
        <b>What does this table show?</b><br>
        This table lists topics that are <span style="color:#388e3c;"><b>significantly associated</b></span> with quiz completion.<br>
        For each topic and quiz pair, a statistical test checks if students who mention a topic are more or less likely to complete a quiz.<br>
        <span style="color:#1976d2;">&#8593; More likely</span> &nbsp; <span style="color:#d32f2f;">&#8595; Less likely</span><br>
        <i>Hover over the effect size for details.</i>
        </div>'''

    def _chi_square_legend_block(self):
        return '''<div style="font-size:0.97em;margin-top:8px;color:#555;">
        <b>Contingency Table Legend:</b><br>
        <table style="border-collapse:collapse;font-size:0.95em;">
          <tr><th></th><th>Completed Quiz</th><th>Did Not Complete Quiz</th></tr>
          <tr><th>Mentioned Topic</th><td>Number of students who mentioned the topic and completed the quiz</td><td>Number of students who mentioned the topic and did not complete the quiz</td></tr>
          <tr><th>Did Not Mention Topic</th><td>Number of students who did not mention the topic but completed the quiz</td><td>Number of students who did not mention the topic and did not complete the quiz</td></tr>
        </table>
        </div>'''

    def _build_quiz_topic_reverse_chi_square_table(self, student_topics, student_quizzes, summary_topics, all_quizzes):
        results = ChiSquareTableHelper.build_quiz_topic_reverse_chi_square(student_topics, student_quizzes, summary_topics, all_quizzes)
        html = '''<div style="margin-bottom:16px; font-size:1.1em;">
        <b>Reverse Analysis: Does completing a support quiz make students less likely to mention the associated challenge?</b><br>
        This table shows, for each quiz-topic pair, whether students who completed the quiz are <span style="color:#d32f2f;"><b>less likely</b></span> (or more likely) to mention the challenge.<br>
        <span style="color:#1976d2;">&#8593; More likely</span> &nbsp; <span style="color:#d32f2f;">&#8595; Less likely</span><br>
        <i>Hover over the effect size for details.</i>
        </div>'''
        html += '''<details style="margin-top:32px;"><summary style="font-size:1.2em;cursor:pointer;"><b>Quiz Completion and Likelihood of Mentioning Challenge (Reverse Chi-Square)</b></summary>'''
        html += '<div class="table-responsive">'
        html += '<table class="emotions-topics-table"><thead><tr><th>Quiz</th><th>Topic</th><th>p-value</th><th>Effect Size (Odds Ratio)</th><th>Direction</th><th>Contingency Table</th></tr></thead><tbody>'
        for quiz, topic, p, table, or_value in results:
            if or_value < 1:
                direction = '<span style="color:#d32f2f;">&#8595; Less likely</span>'
            elif or_value > 1:
                direction = '<span style="color:#1976d2;">&#8593; More likely</span>'
                ...
                direction = 'No effect'
            or_html = f'<span title="Odds Ratio quantifies how much more likely (or less likely) students who completed the quiz are to mention the challenge compared to those who did not. OR < 1: less likely, OR > 1: more likely.">{or_value:.2f}</span>'
            subtable = ChiSquareTableHelper.render_contingency_subtable(
                table,
                row_labels=["Completed Quiz", "Did Not Complete Quiz"],
                col_labels=["Mentioned Topic", "Did Not Mention Topic"]
            )
            html += f'<tr><td>{quiz}</td><td>{topic}</td><td>{p:.5f}</td><td>{or_html}</td><td>{direction}</td><td>{subtable}</td></tr>'
        html += '</tbody></table>'
        html += '</div>'
        html += '''<div style="font-size:0.97em;margin-top:8px;color:#555;">
        <b>Contingency Table Legend:</b><br>
        <table style="border-collapse:collapse;font-size:0.95em;">
          <tr><th></th><th>Mentioned Topic</th><th>Did Not Mention Topic</th></tr>
          <tr><th>Completed Quiz</th><td>Number of students who completed the quiz and mentioned the topic</td><td>Number of students who completed the quiz and did not mention the topic</td></tr>
          <tr><th>Did Not Complete Quiz</th><td>Number of students who did not complete the quiz but mentioned the topic</td><td>Number of students who did not complete the quiz and did not mention the topic</td></tr>
        </table>
        </div></details>'''
        return html 
            ...
            if os.path.exists(csv_path):
                ...
                for _, row in df.iterrows():
                    ...
                    if os.path.exists(topic_csv):
                        try:
                            topic_df = pd.read_csv(topic_csv)
                            # For email IDs, match by extracting username part and comparing with exploded data
                            if is_email_id:
                                username = student_id.split('@')[0].lower()
                                # Try exact email match first, then username match
                                topic_rows = topic_df[topic_df['ID'].astype(str).str.strip().str.lower() == student_id.lower()]
                                if topic_rows.empty:
                                    # Try username-based matching for exploded data that might have different formats
                                    topic_rows = topic_df[topic_df['ID'].astype(str).str.strip().str.lower().str.startswith(username)]
                            else:
                                # Legacy 4-digit code matching
                                topic_rows = topic_df[topic_df['ID'].astype(str).str.strip().str.lower() == student_id.lower()]
                                if topic_rows.empty and student_id.isdigit() and len(student_id) == 4:
                                    topic_rows = topic_df[topic_df['ID'].astype(str).str.strip().str.startswith(student_id)]
    ...
    def _build_grade_chart_from_csv(self, course, csv_path):
        """
        Build the Average Grade Over Time chart using data from the exported CSV file.
        This is a cleaner, more efficient approach than processing multiple files.
        """
        if not os.path.exists(csv_path):
            return '<div style="color:red;">CSV file not found. Please ensure the emotions/topics table has been generated first.</div>'
        
        try:
            # Read the exported CSV
            df = pd.read_csv(csv_path)
            print(f"DEBUG CHART: Loaded CSV with {len(df)} rows and columns: {list(df.columns)}")
            
            # Extract reflection numbers from column names
            grade_columns = [col for col in df.columns if col.startswith('Ref') and col.endswith('_Grade')]
            reflection_numbers = sorted([int(col.split('_')[0].replace('Ref', '')) for col in grade_columns])
            
            print(f"DEBUG CHART: Found grade columns: {grade_columns}")
            print(f"DEBUG CHART: Reflection numbers: {reflection_numbers}")
            
            if not reflection_numbers:
                return '<div style="color:red;">No grade data found in CSV file.</div>'
            
            # Calculate overall average grades (all students only)
            avg_grade_data = []
            
            for ref_num in reflection_numbers:
                grade_col = f'Ref{ref_num}_Grade'
                
                if grade_col in df.columns:
                    # All students average - exclude "-" values more explicitly
                    all_grades_raw = df[grade_col].replace(['-', '', 'N/A', 'n/a'], pd.NA)
                    grades = pd.to_numeric(all_grades_raw, errors='coerce').dropna()
                    
                    print(f"DEBUG CHART: Reflection {ref_num} - Raw grades count: {len(df[grade_col])}")
                    print(f"DEBUG CHART: Reflection {ref_num} - Valid numeric grades count: {len(grades)}")
                    print(f"DEBUG CHART: Reflection {ref_num} - Sample raw grades: {df[grade_col].head(10).tolist()}")
                    if not grades.empty:
                        print(f"DEBUG CHART: Reflection {ref_num} - Sample valid grades: {grades.head(10).tolist()}")
                        print(f"DEBUG CHART: Reflection {ref_num} - Grade statistics: min={grades.min():.2f}, max={grades.max():.2f}, mean={grades.mean():.2f}")
                    
                    all_avg = grades.mean() if not grades.empty else None  # Use None instead of 0.0 for missing data
                    all_count = len(grades)
                    
                    # Only add data point if we have valid grades
                    if all_avg is not None:
                        avg_grade_data.append({
                            'Reflection': ref_num, 
                            'AverageGrade': round(all_avg, 2), 
                            'Count': all_count
                        })
                        print(f"DEBUG CHART: Added all students data point for ref{ref_num}: avg={all_avg:.2f}, count={all_count}")
                    else:
                        print(f"WARNING: No valid grade data found for Reflection {ref_num} - skipping from chart")
            ...
            if avg_grade_data:
                ...
                <div style="background-color: #e3f2fd; border: 1px solid #90caf9; padding: 10px; margin-bottom: 15px; border-radius: 5px;">
                    ...
                    • Shows average grades for all students with valid grade data across reflections<br>
                    • Includes both reflection submitters and non-submitters from the course roster<br>
                ...
                return '<div style="color:orange;">No grade data available for chart generation.</div>'
                
        except Exception as e:
            return f'<div style="color:red;">Error reading CSV file: {str(e)}</div>'
    ...
    def _process_student_reflection_data(self, student_id, ref_num, csv_path, emotions, topics, 
        ...
        if grade == "-":
            ...
            # Get extracted topics from topic analysis
            extracted_topics = []
            topic_csv = os.path.join(os.path.dirname(csv_path), 'results', f'{course.course_name}_ref{ref_num}_exploded.csv')
            if os.path.exists(topic_csv):
                try:
